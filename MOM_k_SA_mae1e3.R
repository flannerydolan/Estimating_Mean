...
#mae<-function(x){lmom::samlmu(x,nmom=2)[2]}
...
mae %>% gather(key=Distribution,value=mae,Lognormal:`Power Law`)->mae

mae %>% saveRDS("MOMkSA_n1000_mae.rds")

ggplot(mae,aes(Groups,mae,col=LCV,linetype=Distribution))+theme_bw()+coord_cartesian(ylim=c(0,1))+ylab("Mean Absolute Error")+
  geom_line(size=1)+ggtitle("n=1,000, m=1,000")+scale_color_manual(values=c("black","grey","red"))+scale_x_continuous(trans='log10')

ggsave("MOMkSA_n1000_logged_mae.png",dpi=300)
