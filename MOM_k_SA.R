...
RMSE %>% gather(key=Distribution,value=RMSE,Lognormal:`Power Law`)->RMSE

ggplot(RMSE,aes(log10(Groups),RMSE,col=LCV,linetype=Distribution))+theme_bw()+coord_cartesian(ylim=c(0,1))+
  geom_line(size=1)+ggtitle("n=100, m=1,000")+scale_color_manual(values=c("black","grey","red"))

ggsave("MOMkSA_n102_logged.png",dpi=300)
